...
# MAGIC %md
# MAGIC # 01 — Bronze → Silver (Databricks Free Edition)
# MAGIC
# MAGIC **Author:** Mathi Sankar M R · PipelineX
# MAGIC
# MAGIC Local variant of `01_bronze_to_silver.py`. Reads CSVs from a Unity
# MAGIC Catalog Volume instead of ADLS, writes Delta to `/tmp/pipelinex/silver/`.
...
def csv_to_silver(filename: str, name: str, schema, dedup_keys):
    src = f"{RAW_VOL}/{filename}"
    dst = f"{SILVER}/{name}"
    df = (
        spark.read
        .option("header", True)
